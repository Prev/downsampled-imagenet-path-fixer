...
def load_training_databatch(data_folder, idx, img_size=32, normalize=False,
                            create_mirrored_images=False):
    """
    Load training databatch using numpy.
    Source referenced from
    https://github.com/PatrykChrabaszcz/Imagenet32_Scripts/blob/master/WRNs_imagenet.py
    """
    ...
    # Labels are indexed from 1, shift it so that indexes start from 0
    ...
    if create_mirrored_images:
        x_flipped = x[:, :, :, ::-1]
        y_flipped = y
        x = np.concatenate((x, x_flipped), axis=0)
        y = np.concatenate((y, y_flipped), axis=0)

    images = lasagne.utils.floatX(x)
    labels = y.astype('int32')
    return images, labels, mean_image


def load_validation_databatch(data_folder, img_size=32, normalize=False, mean_image=None):
...
def saveimg2file(data, filepath):
    """ Save numpy image data to file
    """
    # Fix dim
...
def main(dataset_path, output_path, img_size, normalize, create_mirrored_images=False):
    print(f'Run downsampled imagenet path fixer\n'
          f' - Dataset path: {dataset_path}\n'
          f' - Output path: {output_path}\n'
          f' - Image Size: {img_size}\n'
          f' - Normalize: {normalize}\n'
          f' - Create mirrored images: {create_mirrored_images}\n'
    )
    ...
    # Get number of databatches
    num_databatches = 0
    for file in os.listdir(dataset_path):
        if 'train_data_batch' in file:
            num_databatches += 1

    # Load training databatches
    for batch in range(num_databatches):
        print(f'--- Load Batch {batch+1} ---')
        images, labels, _ = load_training_databatch(dataset_path, batch+1, img_size, normalize,
                                                    create_mirrored_images)

        for i in range(len(labels)):
            filepath = os.path.join(output_path, 'train', wnids[labels[i]], f'{image_id}.jpeg')
            saveimg2file(images[i], filepath)

            if i % 50000 == 0:
                print(f'[{i}/{len(labels)}] {filepath}')
            image_id += 1

        del images
        del labels

    # Caculcate mean value if performing normalization
    if normalize:
        mean_image = unpickle(os.path.join(dataset_path, 'train_data_batch_1'))['mean']
        mean_image /= np.float32(255)
    else:
        mean_image = None

    # Load validation databatche
    images, labels = load_validation_databatch(dataset_path, img_size, normalize, mean_image)

    for i in range(len(labels)):
        filepath = os.path.join(output_path, 'val', wnids[labels[i]], f'{image_id}.jpeg')
        saveimg2file(images[i], filepath)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Downsampled ImageNet Path Fixer')
    parser.add_argument('-d', '--datapath', help='path to downloaed dataset', required=True)
    parser.add_argument('-o', '--outdir', help='path to output directory', required=True)
    parser.add_argument('-s', '--imgsize', default=32, type=int,
                        help='size of image (default: 32)')
    parser.add_argument('-n', '--normalize', default=False, type=bool,
                        help='whether normalize images (default: False)')
    parser.add_argument('-m', '--mirrored', default=False, type=bool,
                        help='whether create mirrored images (default: False)')
    args = parser.parse_args()

    main(args.datapath, args.outdir, args.imgsize, args.normalize, args.mirrored)
